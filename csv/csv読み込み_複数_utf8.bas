Sub read_csvs_utf8()
    ...
    'ファイル取得入荷数
    csvFilePath_shiji = "\\Afnewt320-kyoyu\社内共有\AFSKS\在庫表\csv\入荷数\"
    file_list_shiji = csvファイル名探索(csvFilePath_shiji)

    'csv貼り付け_入荷数
    sh_name = "入荷数csv"
    Dim nyuka_data As Variant
    nyuka_data = getCSV_utf8(sh_name, file_list_shiji, csvFilePath_shiji)
...
Function csvファイル名探索(csvFilePath As Variant) As Variant
    ...
    With CreateObject("Scripting.FileSystemObject")
        For Each f In .GetFolder(csvFilePath).Files
            If f.Name Like "*その他*" Or f.Name Like "*肉*" Or f.Name Like "*日畜トラック有無*" Then
                s = s + 1
            End If
        Next f
    End With
